          ...
          arrRequests[i].onreadystatechange = function (event) {
            if (arrRequests[i].readyState === 4) {
              if (arrRequests[i].status === 200) {
                ...
                setTimeout(function(){ markIPLocation(ipData); }, i*75);
              } else {
...
const displayCountryChart = function(){
  ...
  const options = {
    ...
    // chartArea: {right:700, top:70, width:'100%', height:'100%'},
    is3D: true,
  };
  ...
  function countrySelect() {
    ...
    if (selectedItem){
      ...
      tableChart.draw(tableData, {showRowNumber: true, width: '100%', height: '100%'});
